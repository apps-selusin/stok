...
class ct11_jual_list extends ct11_jual {
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->TglSO->AdvancedSearch->ToJson(), ","); // Field TglSO
		$sFilterList = ew_Concat($sFilterList, $this->NoSO->AdvancedSearch->ToJson(), ","); // Field NoSO
		$sFilterList = ew_Concat($sFilterList, $this->CustomerID->AdvancedSearch->ToJson(), ","); // Field CustomerID
		$sFilterList = ew_Concat($sFilterList, $this->CustomerPO->AdvancedSearch->ToJson(), ","); // Field CustomerPO
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "ft11_juallistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field TglSO
		$this->TglSO->AdvancedSearch->SearchValue = @$filter["x_TglSO"];
		$this->TglSO->AdvancedSearch->SearchOperator = @$filter["z_TglSO"];
		$this->TglSO->AdvancedSearch->SearchCondition = @$filter["v_TglSO"];
		$this->TglSO->AdvancedSearch->SearchValue2 = @$filter["y_TglSO"];
		$this->TglSO->AdvancedSearch->SearchOperator2 = @$filter["w_TglSO"];
		$this->TglSO->AdvancedSearch->Save();

		// Field NoSO
		$this->NoSO->AdvancedSearch->SearchValue = @$filter["x_NoSO"];
		$this->NoSO->AdvancedSearch->SearchOperator = @$filter["z_NoSO"];
		$this->NoSO->AdvancedSearch->SearchCondition = @$filter["v_NoSO"];
		$this->NoSO->AdvancedSearch->SearchValue2 = @$filter["y_NoSO"];
		$this->NoSO->AdvancedSearch->SearchOperator2 = @$filter["w_NoSO"];
		$this->NoSO->AdvancedSearch->Save();
		...
		// Field CustomerPO
		$this->CustomerPO->AdvancedSearch->SearchValue = @$filter["x_CustomerPO"];
		$this->CustomerPO->AdvancedSearch->SearchOperator = @$filter["z_CustomerPO"];
		$this->CustomerPO->AdvancedSearch->SearchCondition = @$filter["v_CustomerPO"];
		$this->CustomerPO->AdvancedSearch->SearchValue2 = @$filter["y_CustomerPO"];
		$this->CustomerPO->AdvancedSearch->SearchOperator2 = @$filter["w_CustomerPO"];
		$this->CustomerPO->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->NoSO, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->CustomerPO, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ft11_juallistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
...
var CurrentSearchForm = ft11_juallistsrch = new ew_Form("ft11_juallistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($t11_jual_list->SearchOptions->Visible()) { ?>
<?php $t11_jual_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($t11_jual_list->FilterOptions->Visible()) { ?>
<?php $t11_jual_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php
	...
	if ($t11_jual_list->AuditTrailOnSearch && $t11_jual_list->Command == "search" && !$t11_jual_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $t11_jual_list->getSessionWhere();
		$t11_jual_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$t11_jual_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($t11_jual->Export == "" && $t11_jual->CurrentAction == "") { ?>
<form name="ft11_juallistsrch" id="ft11_juallistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($t11_jual_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ft11_juallistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="t11_jual">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($t11_jual_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($t11_jual_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $t11_jual_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($t11_jual_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($t11_jual_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($t11_jual_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($t11_jual_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php if ($t11_jual->NoSO->Visible) { // NoSO ?>
	...
	<?php } else { ?>
		<th data-name="NoSO" class="<?php echo $t11_jual->NoSO->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t11_jual->SortUrl($t11_jual->NoSO) ?>',2);"><div id="elh_t11_jual_NoSO" class="t11_jual_NoSO">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t11_jual->NoSO->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t11_jual->NoSO->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t11_jual->NoSO->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($t11_jual->CustomerPO->Visible) { // CustomerPO ?>
	...
	<?php } else { ?>
		<th data-name="CustomerPO" class="<?php echo $t11_jual->CustomerPO->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t11_jual->SortUrl($t11_jual->CustomerPO) ?>',2);"><div id="elh_t11_jual_CustomerPO" class="t11_jual_CustomerPO">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t11_jual->CustomerPO->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t11_jual->CustomerPO->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t11_jual->CustomerPO->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($t11_jual->Export == "") { ?>
<script type="text/javascript">
ft11_juallistsrch.FilterList = <?php echo $t11_jual_list->GetFilterList() ?>;
ft11_juallistsrch.Init();
ft11_juallist.Init();
</script>
<?php } ?>
<?php
