...
$t10_hutangdetail_list = NULL; // Initialize page object first

class ct10_hutangdetail_list extends ct10_hutangdetail {
	...
	// Page object name
	var $PageObjName = 't10_hutangdetail_list';

	// Grid form hidden field names
	var $FormName = 'ft10_hutangdetaillist';
	...
	function __construct() {
		...
		$this->AddUrl = "t10_hutangdetailadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "t10_hutangdetaildelete.php";
		$this->MultiUpdateUrl = "t10_hutangdetailupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft10_hutangdetaillistsrch";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t09_hutang") {
			global $t09_hutang;
			$rsmaster = $t09_hutang->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("t09_hutanglist.php"); // Return to master page
			} else {
				$t09_hutang->LoadListRowValues($rsmaster);
				$t09_hutang->RowType = EW_ROWTYPE_MASTER; // Master row
				$t09_hutang->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->HutangID->AdvancedSearch->ToJson(), ","); // Field HutangID
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "ft10_hutangdetaillistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field HutangID
		$this->HutangID->AdvancedSearch->SearchValue = @$filter["x_HutangID"];
		$this->HutangID->AdvancedSearch->SearchOperator = @$filter["z_HutangID"];
		$this->HutangID->AdvancedSearch->SearchCondition = @$filter["v_HutangID"];
		$this->HutangID->AdvancedSearch->SearchValue2 = @$filter["y_HutangID"];
		$this->HutangID->AdvancedSearch->SearchOperator2 = @$filter["w_HutangID"];
		$this->HutangID->AdvancedSearch->Save();
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.ft10_hutangdetaillist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft10_hutangdetaillistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft10_hutangdetaillistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft10_hutangdetaillist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ft10_hutangdetaillistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_t10_hutangdetail\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_t10_hutangdetail',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ft10_hutangdetaillist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t09_hutang") {
			global $t09_hutang;
			if (!isset($t09_hutang)) $t09_hutang = new ct09_hutang;
			$rsmaster = $t09_hutang->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$t09_hutang;
					$t09_hutang->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
// Create page object
if (!isset($t10_hutangdetail_list)) $t10_hutangdetail_list = new ct10_hutangdetail_list();

// Page init
$t10_hutangdetail_list->Page_Init();

// Page main
$t10_hutangdetail_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t10_hutangdetail_list->Page_Render();
?>
...
ft10_hutangdetaillist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = ft10_hutangdetaillistsrch = new ew_Form("ft10_hutangdetaillistsrch");
</script>
<script type="text/javascript">
...
<?php if ($t10_hutangdetail->Export == "") { ?>
<div class="ewToolbar">
<?php if ($t10_hutangdetail_list->TotalRecs > 0 && $t10_hutangdetail_list->ExportOptions->Visible()) { ?>
<?php $t10_hutangdetail_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($t10_hutangdetail_list->SearchOptions->Visible()) { ?>
<?php $t10_hutangdetail_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($t10_hutangdetail_list->FilterOptions->Visible()) { ?>
<?php $t10_hutangdetail_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if (($t10_hutangdetail->Export == "") || (EW_EXPORT_MASTER_RECORD && $t10_hutangdetail->Export == "print")) { ?>
<?php
if ($t10_hutangdetail_list->DbMasterFilter <> "" && $t10_hutangdetail->getCurrentMasterTable() == "t09_hutang") {
	if ($t10_hutangdetail_list->MasterRecordExists) {
?>
<?php include_once "t09_hutangmaster.php" ?>
<?php
	}
}
?>
<?php } ?>
<?php
	$bSelectLimit = $t10_hutangdetail_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($t10_hutangdetail_list->TotalRecs <= 0)
			$t10_hutangdetail_list->TotalRecs = $t10_hutangdetail->ListRecordCount();
	} else {
		if (!$t10_hutangdetail_list->Recordset && ($t10_hutangdetail_list->Recordset = $t10_hutangdetail_list->LoadRecordset()))
			$t10_hutangdetail_list->TotalRecs = $t10_hutangdetail_list->Recordset->RecordCount();
	}
	$t10_hutangdetail_list->StartRec = 1;
	if ($t10_hutangdetail_list->DisplayRecs <= 0 || ($t10_hutangdetail->Export <> "" && $t10_hutangdetail->ExportAll)) // Display all records
		$t10_hutangdetail_list->DisplayRecs = $t10_hutangdetail_list->TotalRecs;
	if (!($t10_hutangdetail->Export <> "" && $t10_hutangdetail->ExportAll))
		$t10_hutangdetail_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t10_hutangdetail_list->Recordset = $t10_hutangdetail_list->LoadRecordset($t10_hutangdetail_list->StartRec-1, $t10_hutangdetail_list->DisplayRecs);

	// Set no record found message
	if ($t10_hutangdetail->CurrentAction == "" && $t10_hutangdetail_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$t10_hutangdetail_list->setWarningMessage(ew_DeniedMsg());
		if ($t10_hutangdetail_list->SearchWhere == "0=101")
			$t10_hutangdetail_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$t10_hutangdetail_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($t10_hutangdetail_list->AuditTrailOnSearch && $t10_hutangdetail_list->Command == "search" && !$t10_hutangdetail_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $t10_hutangdetail_list->getSessionWhere();
		$t10_hutangdetail_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$t10_hutangdetail_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($t10_hutangdetail->Export == "" && $t10_hutangdetail->CurrentAction == "") { ?>
<form name="ft10_hutangdetaillistsrch" id="ft10_hutangdetaillistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($t10_hutangdetail_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ft10_hutangdetaillistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="t10_hutangdetail">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($t10_hutangdetail_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($t10_hutangdetail_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $t10_hutangdetail_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($t10_hutangdetail_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($t10_hutangdetail_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($t10_hutangdetail_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($t10_hutangdetail_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $t10_hutangdetail_list->ShowPageHeader(); ?>
<?php
$t10_hutangdetail_list->ShowMessage();
?>
<?php if ($t10_hutangdetail_list->TotalRecs > 0 || $t10_hutangdetail->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($t10_hutangdetail_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> t10_hutangdetail">
<?php if ($t10_hutangdetail->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($t10_hutangdetail->CurrentAction <> "gridadd" && $t10_hutangdetail->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t10_hutangdetail_list->Pager)) $t10_hutangdetail_list->Pager = new cPrevNextPager($t10_hutangdetail_list->StartRec, $t10_hutangdetail_list->DisplayRecs, $t10_hutangdetail_list->TotalRecs, $t10_hutangdetail_list->AutoHidePager) ?>
<?php if ($t10_hutangdetail_list->Pager->RecordCount > 0 && $t10_hutangdetail_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t10_hutangdetail_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t10_hutangdetail_list->PageUrl() ?>start=<?php echo $t10_hutangdetail_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t10_hutangdetail_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t10_hutangdetail_list->PageUrl() ?>start=<?php echo $t10_hutangdetail_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t10_hutangdetail_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t10_hutangdetail_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t10_hutangdetail_list->PageUrl() ?>start=<?php echo $t10_hutangdetail_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t10_hutangdetail_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t10_hutangdetail_list->PageUrl() ?>start=<?php echo $t10_hutangdetail_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t10_hutangdetail_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($t10_hutangdetail_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $t10_hutangdetail_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $t10_hutangdetail_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $t10_hutangdetail_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($t10_hutangdetail_list->TotalRecs > 0 && (!$t10_hutangdetail_list->AutoHidePageSizeSelector || $t10_hutangdetail_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="t10_hutangdetail">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($t10_hutangdetail_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($t10_hutangdetail_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($t10_hutangdetail_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($t10_hutangdetail_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="200"<?php if ($t10_hutangdetail_list->DisplayRecs == 200) { ?> selected<?php } ?>>200</option>
<option value="ALL"<?php if ($t10_hutangdetail->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($t10_hutangdetail_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="ft10_hutangdetaillist" id="ft10_hutangdetaillist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t10_hutangdetail_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t10_hutangdetail_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t10_hutangdetail">
<?php if ($t10_hutangdetail->getCurrentMasterTable() == "t09_hutang" && $t10_hutangdetail->CurrentAction <> "") { ?>
...
// Header row
$t10_hutangdetail_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($t10_hutangdetail->HutangID->Visible) { // HutangID ?>
	...
	<?php } else { ?>
		<th data-name="HutangID" class="<?php echo $t10_hutangdetail->HutangID->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t10_hutangdetail->SortUrl($t10_hutangdetail->HutangID) ?>',2);"><div id="elh_t10_hutangdetail_HutangID" class="t10_hutangdetail_HutangID">
...
<?php if ($t10_hutangdetail->NoBayar->Visible) { // NoBayar ?>
	...
	<?php } else { ?>
		<th data-name="NoBayar" class="<?php echo $t10_hutangdetail->NoBayar->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t10_hutangdetail->SortUrl($t10_hutangdetail->NoBayar) ?>',2);"><div id="elh_t10_hutangdetail_NoBayar" class="t10_hutangdetail_NoBayar">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t10_hutangdetail->NoBayar->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t10_hutangdetail->NoBayar->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t10_hutangdetail->NoBayar->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($t10_hutangdetail->Tgl->Visible) { // Tgl ?>
	...
	<?php } else { ?>
		<th data-name="Tgl" class="<?php echo $t10_hutangdetail->Tgl->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t10_hutangdetail->SortUrl($t10_hutangdetail->Tgl) ?>',2);"><div id="elh_t10_hutangdetail_Tgl" class="t10_hutangdetail_Tgl">
...
<?php if ($t10_hutangdetail->JumlahBayar->Visible) { // JumlahBayar ?>
	...
	<?php } else { ?>
		<th data-name="JumlahBayar" class="<?php echo $t10_hutangdetail->JumlahBayar->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t10_hutangdetail->SortUrl($t10_hutangdetail->JumlahBayar) ?>',2);"><div id="elh_t10_hutangdetail_JumlahBayar" class="t10_hutangdetail_JumlahBayar">
...
$t10_hutangdetail_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
...
if ($t10_hutangdetail->ExportAll && $t10_hutangdetail->Export <> "") {
	...
	// Set the last record to display
	if ($t10_hutangdetail_list->TotalRecs > $t10_hutangdetail_list->StartRec + $t10_hutangdetail_list->DisplayRecs - 1)
...
if ($t10_hutangdetail_list->Recordset && !$t10_hutangdetail_list->Recordset->EOF) {
	$t10_hutangdetail_list->Recordset->MoveFirst();
	$bSelectLimit = $t10_hutangdetail_list->UseSelectLimit;
	if (!$bSelectLimit && $t10_hutangdetail_list->StartRec > 1)
		$t10_hutangdetail_list->Recordset->Move($t10_hutangdetail_list->StartRec - 1);
} elseif (!$t10_hutangdetail->AllowAddDeleteRow && $t10_hutangdetail_list->StopRec == 0) {
	$t10_hutangdetail_list->StopRec = $t10_hutangdetail->GridAddRowCount;
}
...
<?php
	...
	if ($t10_hutangdetail->CurrentAction <> "gridadd")
		$t10_hutangdetail_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
...
// Close recordset
if ($t10_hutangdetail_list->Recordset)
	$t10_hutangdetail_list->Recordset->Close();
?>
<?php if ($t10_hutangdetail->Export == "") { ?>
...
<?php } ?>
<?php if ($t10_hutangdetail_list->TotalRecs == 0 && $t10_hutangdetail->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($t10_hutangdetail_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($t10_hutangdetail->Export == "") { ?>
<script type="text/javascript">
ft10_hutangdetaillistsrch.FilterList = <?php echo $t10_hutangdetail_list->GetFilterList() ?>;
ft10_hutangdetaillistsrch.Init();
ft10_hutangdetaillist.Init();
</script>
<?php } ?>
<?php
$t10_hutangdetail_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($t10_hutangdetail->Export == "") { ?>
...
<?php
$t10_hutangdetail_list->Page_Terminate();
?>
