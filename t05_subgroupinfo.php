...
//
// Table class for t05_subgroup
//
class ct05_subgroup extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't05_subgroup';
		$this->TableName = 't05_subgroup';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t05_subgroup`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('t05_subgroup', 't05_subgroup', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// MainGroupID
		$this->MainGroupID = new cField('t05_subgroup', 't05_subgroup', 'x_MainGroupID', 'MainGroupID', '`MainGroupID`', '`MainGroupID`', 3, -1, FALSE, '`EV__MainGroupID`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// Kode
		$this->Kode = new cField('t05_subgroup', 't05_subgroup', 'x_Kode', 'Kode', '`Kode`', '`Kode`', 200, -1, FALSE, '`Kode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Kode->Sortable = TRUE; // Allow sort
		$this->fields['Kode'] = &$this->Kode;

		// Nama
		$this->Nama = new cField('t05_subgroup', 't05_subgroup', 'x_Nama', 'Nama', '`Nama`', '`Nama`', 200, -1, FALSE, '`Nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(COALESCE(`Kode`, ''),'" . ew_ValueSeparator(1, $this->MainGroupID) . "',COALESCE(`Nama`,'')) FROM `t04_maingroup` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `t05_subgroup`.`MainGroupID` LIMIT 1) AS `EV__MainGroupID` FROM `t05_subgroup`" .
			") `EW_TMP_TABLE`";
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "t05_subgroupedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "t05_subgroupadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t05_subgroupview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t05_subgroupview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t05_subgroupadd.php?" . $this->UrlParm($parm);
		else
			$url = "t05_subgroupadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t05_subgroupedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t05_subgroupadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t05_subgroupdelete.php", $this->UrlParm());
	}
