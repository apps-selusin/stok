...
//		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if (!$rs->EOF) {
			...
			$this->Val[1] = $this->TglPO->CurrentValue;
			$this->Val[2] = $this->NoPO->CurrentValue;
			$this->Val[3] = $this->VendorID->CurrentValue;
			$this->Val[4] = $this->ArticleID->CurrentValue;
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_t08_po_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_t08_po_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_t08_po_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_t08_po_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_t08_po_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_t08_po_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_t08_po_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_t08_po_' . $parm] = $sv;
		$_SESSION['so_t08_po_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_t08_po_' . $parm] = $sv1;
		$_SESSION['so_t08_po_' . $parm] = $so1;
		$_SESSION['sc_t08_po_' . $parm] = $sc;
		$_SESSION['sv2_t08_po_' . $parm] = $sv2;
		$_SESSION['so2_t08_po_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_t08_po_$parm"] = "";
		$_SESSION["rf_t08_po_$parm"] = "";
		$_SESSION["rt_t08_po_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_t08_po_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_t08_po_$parm"];
		$fld->RangeTo = @$_SESSION["rt_t08_po_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field TglPO
		$this->SetDefaultExtFilter($this->TglPO, "=", NULL, 'AND', "=", NULL);
...
// Form object
var CurrentForm = ft08_porpt = new ewr_Form("ft08_porpt");

// Validate method
ft08_porpt.Validate = function() {
...
ft08_porpt.Form_CustomValidate = 
...
ft08_porpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
ft08_porpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="ft08_porpt" id="ft08_porpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="ft08_porpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<div id="c_TglPO" class="ewCell form-group">
	...
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("="); ?><input type="hidden" name="so_TglPO" id="so_TglPO" value="="></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->TglPO->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="t08_po" data-field="x_TglPO" id="sv_TglPO" name="sv_TglPO" placeholder="<?php echo $Page->TglPO->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->TglPO->SearchValue) ?>" data-calendar='true' data-options='{"ignoreReadonly":true,"useCurrent":false,"format":7}'<?php echo $Page->TglPO->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
ft08_porpt.Init();
ft08_porpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
