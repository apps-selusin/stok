...
<?php include_once "r01_pembeliansmryinfo.php" ?>
<?php
...
$r01_pembelian_summary = NULL; // Initialize page object first

class crr01_pembelian_summary extends crr01_pembelian {
	...
	// Page object name
	var $PageObjName = 'r01_pembelian_summary';
	...
	function __construct() {
		...
		// Table object (r01_pembelian)
		if (!isset($GLOBALS["r01_pembelian"])) {
			$GLOBALS["r01_pembelian"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r01_pembelian"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r01_pembelian', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr01_pembeliansummary";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r01_pembelian');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r01_pembelian\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r01_pembelian',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr01_pembeliansummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr01_pembeliansummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr01_pembeliansummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r01_pembelian_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r01_pembelian_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r01_pembelian_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r01_pembelian_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r01_pembelian_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r01_pembelian_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r01_pembelian_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r01_pembelian_' . $parm] = $sv;
		$_SESSION['so_r01_pembelian_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r01_pembelian_' . $parm] = $sv1;
		$_SESSION['so_r01_pembelian_' . $parm] = $so1;
		$_SESSION['sc_r01_pembelian_' . $parm] = $sc;
		$_SESSION['sv2_r01_pembelian_' . $parm] = $sv2;
		$_SESSION['so2_r01_pembelian_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r01_pembelian_$parm"] = "";
		$_SESSION["rf_r01_pembelian_$parm"] = "";
		$_SESSION["rt_r01_pembelian_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r01_pembelian_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r01_pembelian_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r01_pembelian_$parm"];
	}
...
// Create page object
if (!isset($r01_pembelian_summary)) $r01_pembelian_summary = new crr01_pembelian_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r01_pembelian_summary;
...
// Create page object
var r01_pembelian_summary = new ewr_Page("r01_pembelian_summary");

// Page properties
r01_pembelian_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r01_pembelian_summary.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fr01_pembeliansummary = new ewr_Form("fr01_pembeliansummary");

// Validate method
fr01_pembeliansummary.Validate = function() {
...
fr01_pembeliansummary.Form_CustomValidate = 
...
fr01_pembeliansummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr01_pembeliansummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="fr01_pembeliansummary" id="fr01_pembeliansummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr01_pembeliansummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="r01_pembelian" data-field="x_TglPO" id="sv_TglPO" name="sv_TglPO" placeholder="<?php echo $Page->TglPO->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->TglPO->SearchValue) ?>" data-calendar='true' data-options='{"ignoreReadonly":true,"useCurrent":false,"format":7}'<?php echo $Page->TglPO->EditAttributes() ?>>
</span>
	<span class="ewSearchCond btw1_TglPO"><?php echo $ReportLanguage->Phrase("AND") ?></span>
	<span class="ewSearchField btw1_TglPO">
<?php ewr_PrependClass($Page->TglPO->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r01_pembelian" data-field="x_TglPO" id="sv2_TglPO" name="sv2_TglPO" placeholder="<?php echo $Page->TglPO->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->TglPO->SearchValue2) ?>" data-calendar='true' data-options='{"ignoreReadonly":true,"useCurrent":false,"format":7}'<?php echo $Page->TglPO->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fr01_pembeliansummary.Init();
fr01_pembeliansummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r01_pembeliansmrypager.php" ?>
...
<?php } ?>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_r01_pembelian"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_r01_pembelian" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TglPO"><div class="r01_pembelian_TglPO"><span class="ewTableHeaderCaption"><?php echo $Page->TglPO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TglPO">
<?php if ($Page->SortUrl($Page->TglPO) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_TglPO">
			<span class="ewTableHeaderCaption"><?php echo $Page->TglPO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_TglPO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TglPO) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TglPO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="NoPO"><div class="r01_pembelian_NoPO"><span class="ewTableHeaderCaption"><?php echo $Page->NoPO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="NoPO">
<?php if ($Page->SortUrl($Page->NoPO) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_NoPO">
			<span class="ewTableHeaderCaption"><?php echo $Page->NoPO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_NoPO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NoPO) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->NoPO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="VendorNama"><div class="r01_pembelian_VendorNama"><span class="ewTableHeaderCaption"><?php echo $Page->VendorNama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="VendorNama">
<?php if ($Page->SortUrl($Page->VendorNama) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_VendorNama">
			<span class="ewTableHeaderCaption"><?php echo $Page->VendorNama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_VendorNama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->VendorNama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->VendorNama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ArticleNama"><div class="r01_pembelian_ArticleNama"><span class="ewTableHeaderCaption"><?php echo $Page->ArticleNama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ArticleNama">
<?php if ($Page->SortUrl($Page->ArticleNama) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_ArticleNama">
			<span class="ewTableHeaderCaption"><?php echo $Page->ArticleNama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_ArticleNama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ArticleNama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ArticleNama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Harga"><div class="r01_pembelian_Harga" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->Harga->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Harga">
<?php if ($Page->SortUrl($Page->Harga) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_Harga" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Harga->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_Harga" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Harga) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Harga->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Qty"><div class="r01_pembelian_Qty" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->Qty->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Qty">
<?php if ($Page->SortUrl($Page->Qty) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_Qty" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Qty->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_Qty" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Qty) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Qty->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="SatuanNama"><div class="r01_pembelian_SatuanNama"><span class="ewTableHeaderCaption"><?php echo $Page->SatuanNama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="SatuanNama">
<?php if ($Page->SortUrl($Page->SatuanNama) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_SatuanNama">
			<span class="ewTableHeaderCaption"><?php echo $Page->SatuanNama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_SatuanNama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->SatuanNama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->SatuanNama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="SubTotal"><div class="r01_pembelian_SubTotal" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->SubTotal->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="SubTotal">
<?php if ($Page->SortUrl($Page->SubTotal) == "") { ?>
		<div class="ewTableHeaderBtn r01_pembelian_SubTotal" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->SubTotal->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r01_pembelian_SubTotal" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->SubTotal) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->SubTotal->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r01_pembelian_TglPO"><span class="ewTableHeaderCaption"><?php echo $Page->TglPO->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->TglPO) == "") { ?>
		<span class="ewSummaryCaption r01_pembelian_TglPO">
			<span class="ewTableHeaderCaption"><?php echo $Page->TglPO->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r01_pembelian_TglPO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TglPO) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TglPO->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->TglPO->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->TglPO->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r01_pembelian_TglPO"<?php echo $Page->TglPO->ViewAttributes() ?>><?php echo $Page->TglPO->GroupViewValue ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r01_pembelian_TglPO"<?php echo $Page->TglPO->ViewAttributes() ?>><?php echo $Page->TglPO->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->NoPO->Visible) { ?>
