...
$t12_jualdetail_grid = NULL; // Initialize page object first

class ct12_jualdetail_grid extends ct12_jualdetail {
	...
	// Page object name
	var $PageObjName = 't12_jualdetail_grid';

	// Grid form hidden field names
	var $FormName = 'ft12_jualdetailgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t12_jualdetail"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_JualID") && $objForm->HasValue("o_JualID") && $this->JualID->CurrentValue <> $this->JualID->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ArticleID") && $objForm->HasValue("o_ArticleID") && $this->ArticleID->CurrentValue <> $this->ArticleID->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_HargaJual") && $objForm->HasValue("o_HargaJual") && $this->HargaJual->CurrentValue <> $this->HargaJual->OldValue)
			return FALSE;
	...
	function RenderRow() {
		...
		$this->SubTotal->ViewCustomAttributes = "";
			...
			$this->HargaJual->EditValue = ew_FormatNumber($this->HargaJual->EditValue, -2, -2, -2, -2);
			$this->HargaJual->OldValue = $this->HargaJual->EditValue;
			}
			...
			$this->SubTotal->EditValue = ew_FormatNumber($this->SubTotal->EditValue, -2, -2, -2, -2);
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "t11_jual") {
			$this->JualID->Visible = FALSE;
			if ($GLOBALS["t11_jual"]->EventCancelled) $this->EventCancelled = TRUE;
		}
